...
- **A Madara chapter list stopping at a round number is probably pruning, not a cap — and HTTP 200 on older chapter URLs proves nothing.** The chapter-list audit following #169 flagged manhuaplus.com as capping lists at the newest 500 rows because one series showed exactly 500 (ch 280–529.1) in the series HTML *and* `ajax/chapters/`, while `chapter-1/` still answered 200. Wrong on both counts: those 200s are soft-404s serving the *series page* (canonical points at the series, no reader content), and a sibling series on the same site lists 3364 rows — no newest-N cap exists; the site simply deletes old chapters, and "exactly 500" was a coincidence. The Yoast `wp-manga-chapters-sitemap*.xml` enumeration is the cheap cross-check (it matched both series' visible lists exactly), and no alternative route recovers pruned chapters anyway: `wp/v2` exposes no manga/chapter routes, `ajax/chapters/` ignores `t`/`per_page`/`offset`/`chapter_page`, and the legacy `manga_get_chapters` admin-ajax action 400s. `cmd/root.go` now warns when a requested range dips below the first listed chapter, which covers pruned series on any site without false-alarming on lists that legitimately hold exactly N rows. The oldest chapters that *are* still listed can be rotten anyway: manhuaplus pages from the 2020 `*.files.wordpress.com` era answer 200 with an HTML page instead of an image, which only `tools/verify-cbz` catches (from ~2021 on they moved to `cdn.manhuaplus.com` and work).
