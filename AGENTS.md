...
- `PlainHTML` (`grabber/plainhtml.go`) is a generic goquery scraper driven by a list of `SiteSelector` entries (CSS selectors for title/rows/chapter/image). It covers tcbonepiecechapters.com (TCB Scans), asurascans.com, zonatmo.org, mangapill.com and dynasty-scans.com. Selector order in the list matters because some sites have very similar markup. `getPlainHTMLImageURL()` also recognizes dynasty-scans' (Dynasty Reader) `var pages = [...]` JSON blob on the reader page — the DOM only ever contains the current page's `<img>` (the rest are swapped in by JS on navigation via that same blob), so the `Image` selector alone would only find one page; the JSON is parsed for the ordered `image` field instead, no browser needed.
