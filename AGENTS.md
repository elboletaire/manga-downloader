...
- `PlainHTML` (`grabber/plainhtml.go`) is a generic goquery scraper driven by a list of `SiteSelector` entries (CSS selectors for title/rows/chapter/image). It covers mangabuddy, tcbonepiecechapters.com (TCB Scans) and asurascans.com. Selector order in the list matters because some sites have very similar markup. mangaread.org is also a classic Madara wordpress theme like `tcb.go`'s sites, but doesn't need `tcb.go`'s ajax-chapters/single-pager dance: the full chapter list ships in the series page HTML and each reader page already contains every page image (`li.wp-manga-chapter`, `img.wp-manga-chapter-img`) — plain `PlainHTML` selectors were enough.
