...
- Site-specific grabbers with their own logic: `inmanga.go`, `mangadex.go` (both API-based, language-aware), `mangabats.go` (JSON chapters API + js image variables), `mangafire.go` (open JSON API: `/api/titles/{hid}` + paginated `/chapters` + `/api/chapters/{id}` for pages, language-aware), `mangak.go` (mangak.io, the mangabuddy rebrand: Next.js site, full chapter list and page-image URLs parsed from the `__NEXT_DATA__` JSON blob — the visible HTML only lists ~6 chapters, and the JSON `number` field is a 1-based ordinal so the real number is parsed from the chapter name), `qimanga.go` (paginated JSON chapters API + SSR reader), `tcb.go` (Madara-based wordpress sites, i.e. lhtranslation.net), `leercapitulo.go` (plain HTML chapter list, but the reader page only decodes its obfuscated page-image blob client-side; uses `browser.GetHTMLWithLocalStorage` to flip the site's own "load all pages" preference before scraping the resulting `<img>` tags — no cloudflare, no cipher reversing needed), `comix.go` (comix.to: no cloudflare either, but its `/api/v1/manga/{hid}/chapters` and `/api/v1/chapters/{id}` JSON endpoints require a rotating per-request token and, even with a valid one, return the body AES-encrypted under an `"e"` key with an `x-enc: 1` header — a real browser sidesteps this entirely since the site's own JS decrypts it into normal DOM; the chapter list is classic `?page=N` pagination and often lists the same chapter number multiple times, one row per scanlation-group upload, so it's deduped by number; the reader virtualizes/lazily-mounts page images via IntersectionObserver, so `browser.GetHTMLWithScroll` progressively scrolls in fractional steps of the growing `scrollHeight` to force every real page to mount before the final snapshot — every 10th "page" in the reader's own counter turns out to be an ad slot with no image container at all, not a missed scroll).
