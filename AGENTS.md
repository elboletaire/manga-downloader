...
- `PlainHTML` (`grabber/plainhtml.go`) is a generic goquery scraper driven by a list of `SiteSelector` entries (CSS selectors for title/rows/chapter/image). It covers mangabuddy, tcbonepiecechapters.com (TCB Scans), asurascans.com and templetoons.com. Selector order in the list matters because some sites have very similar markup.
- templetoons.com (Temple Scan) is a Next.js site, but server-rendered (RSC streaming) with the *full* chapter list in the plain-curl HTML as real `<a>` tags — unlike mangak.io, no `__NEXT_DATA__`/JSON-blob parsing is needed for the chapter listing. Its row links are relative to the series page's own directory with no leading slash (e.g. `href="bl-antidote/chapter-88"`), which the old `BaseUrl()`-prefixing logic mangled into `https://host` + `bl-antidote/...` (missing `/comic/`); fixed generically via `PlainHTML.resolveURL()` (`net/url` `ResolveReference`, RFC-3986-correct for both root-relative and directory-relative hrefs, so it doesn't change behaviour for existing sites). The reader page embeds pages as a flat, double-JSON-escaped `\"pages\":[\"https://...\",...]` array (handled as a new pattern in `getPlainHTMLImageURL`) rather than `<img>` tags. Some of the newest chapters are marked `PREMIUM` (paywalled) — pick an older recent one when testing.
