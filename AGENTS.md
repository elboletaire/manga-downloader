...
- `PlainHTML` (`grabber/plainhtml.go`) is a generic goquery scraper driven by a list of `SiteSelector` entries (CSS selectors for title/rows/chapter/image). It covers mangabuddy, tcbonepiecechapters.com (TCB Scans), asurascans.com, and reader.deathtollscans.net (a FoOlSlide reader — series page is static HTML with a `.list .element` row per chapter, but the reader page only renders the current page's `<img class="open">`; every page's real URL is in a `var pages = [...]` JSON array with a `url` field, parsed via `getPlainHTMLImageURL`'s FoOlSlide branch — a pattern likely reusable for other FoolSlide-based scanlation sites). Selector order in the list matters because some sites have very similar markup.
