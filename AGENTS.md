...
- Site-specific grabbers with their own logic: `inmanga.go`, `mangadex.go` (both API-based, language-aware), `mangabats.go` (JSON chapters API + js image variables), `mangafire.go` (open JSON API: `/api/titles/{hid}` + paginated `/chapters` + `/api/chapters/{id}` for pages, language-aware), `mangak.go` (mangak.io, the mangabuddy rebrand: Next.js site, full chapter list and page-image URLs parsed from the `__NEXT_DATA__` JSON blob — the visible HTML only lists ~6 chapters, and the JSON `number` field is a 1-based ordinal so the real number is parsed from the chapter name), `qimanga.go` (paginated JSON chapters API + SSR reader), `tcb.go` (Madara-based wordpress sites, i.e. lhtranslation.net), `leercapitulo.go` (plain HTML chapter list, but the reader page only decodes its obfuscated page-image blob client-side; uses `browser.GetHTMLWithLocalStorage` to flip the site's own "load all pages" preference before scraping the resulting `<img>` tags — no cloudflare, no cipher reversing needed), `mangadenizi.go` (mangadenizi.net, Turkish, Nuxt SSR: chapter list from `/api/v1/web/manga/{slug}`, pages from `/api/v1/reader/{slug}/{chapterSlug}`, both open, no cloudflare — but the reader images are tile-scrambled (`scramble.method:"tiled-v1"`, a deterministic seeded-PRNG shuffle of a grid×grid tile grid) with the descrambling `grid`/`seed` handed over in the same plaintext JSON response, so it's reversed in Go — decode the webp with `golang.org/x/image/webp`, replay the site's own xorshift32 Fisher-Yates shuffle to redo the tile placement, re-encode as PNG — with zero secret keys or browser needed; see "Per-page post-processing" below for how this plugged into the shared download pipeline; the API's page `width`/`height` fields are sometimes `0` (seen on some manga, e.g. one-piece) so dimensions are always read from the decoded image's own bounds instead).
- **Per-page post-processing**: `grabber.Page` has an optional `Transform func([]byte) ([]byte, error)` field; `downloader.FetchFile`/`FetchChapter` apply it to the raw downloaded bytes right after a successful fetch (a transform error is retried exactly like a failed GET). This is the hook a grabber uses when the CDN bytes aren't ready to pack as-is (mangadenizi's tile-descrambling is the first user) — every other grabber leaves `Transform` nil and is unaffected.
