...
- Site-specific grabbers with their own logic: `inmanga.go`, `mangadex.go` (both API-based, language-aware), `mangabats.go` (JSON chapters API + js image variables), `mangafire.go` (open JSON API: `/api/titles/{hid}` + paginated `/chapters` + `/api/chapters/{id}` for pages, language-aware), `mangak.go` (mangak.io, the mangabuddy rebrand: Next.js site, full chapter list and page-image URLs parsed from the `__NEXT_DATA__` JSON blob — the visible HTML only lists ~6 chapters, and the JSON `number` field is a 1-based ordinal so the real number is parsed from the chapter name), `qimanga.go` (paginated JSON chapters API + SSR reader), `tcb.go` (Madara-based wordpress sites, i.e. lhtranslation.net), `leercapitulo.go` (plain HTML chapter list, but the reader page only decodes its obfuscated page-image blob client-side; uses `browser.GetHTMLWithLocalStorage` to flip the site's own "load all pages" preference before scraping the resulting `<img>` tags — no cloudflare, no cipher reversing needed), `guya.go` (guya.moe, a guyamoe/cubari instance: `guya.moe` 302s to `guya.cubari.moe`, and Go's default `http.Client` follows that transparently; `/api/series/{slug}/` returns the *entire* chapter/page tree — including per-group page filenames — in one call, so `FetchChapters` builds every `GuyaChapter`'s full page list up front and `FetchChapter` makes zero extra requests, just formats `/media/manga/{slug}/chapters/{folder}/{group}/{file}` URLs; group ids are picked via the feed's `preferred_sort` list since some chapters were scanlated by more than one group; `danke.moe` (Danke fürs Lesen) is a *different* instance of the same open-source guyamoe software — https://github.com/kafkaien42/guyamoe, linked from its own footer — so it needed no new grabber, just adding `danke\.moe` to `Guya.Test()`'s domain regex).
