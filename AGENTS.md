...
- `PlainHTML` (`grabber/plainhtml.go`) is a generic goquery scraper driven by a list of `SiteSelector` entries (CSS selectors for title/rows/chapter/image). It covers mangabuddy, tcbonepiecechapters.com (TCB Scans) and asurascans.com. Selector order in the list matters because some sites have very similar markup. madarascans.org (`.com` 301s there) is, despite the name, a WordPress "mangareader"/themesia theme, not Madara — chapter rows are `.ch-item` (`data-ch` gives the number directly, but the `.ch-num` text also parses fine) and its reader already fits the existing `ts_reader.run(...)` fallback in `getPlainHTMLImageURL`. Its ~50 most recent chapters carry a `locked` class (paywalled) and simply have no `ts_reader` blob in the plain HTML — those chapters can't be fetched, so always smoke-test with a chapter number below the lock boundary (find it via the `free`/`locked` class on `.ch-item`), not the latest one.
