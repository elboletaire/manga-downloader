...
- Site-specific grabbers with their own logic: `inmanga.go`, `mangadex.go` (both API-based, language-aware), `mangabats.go` (JSON chapters API + js image variables), `mangafire.go` (open JSON API: `/api/titles/{hid}` + paginated `/chapters` + `/api/chapters/{id}` for pages, language-aware), `mangak.go` (mangak.io, the mangabuddy rebrand: Next.js site, full chapter list and page-image URLs parsed from the `__NEXT_DATA__` JSON blob — the visible HTML only lists ~6 chapters, and the JSON `number` field is a 1-based ordinal so the real number is parsed from the chapter name), `qimanga.go` (paginated JSON chapters API + SSR reader), `tcb.go` (Madara-based wordpress sites, i.e. lhtranslation.net), `leercapitulo.go` (plain HTML chapter list, but the reader page only decodes its obfuscated page-image blob client-side; uses `browser.GetHTMLWithLocalStorage` to flip the site's own "load all pages" preference before scraping the resulting `<img>` tags — no cloudflare, no cipher reversing needed), `utoon.go` (utoon.us, a WordPress "mangaverse" theme site: the series page only server-renders the ~10 newest chapters, the rest comes from a paginated `wp-admin/admin-ajax.php` `mangaverse_load_more` POST action using a category id + per-page nonce scraped from the series HTML; the theme's own pagination overlaps by a couple of chapters on the last page or two, so chapters are deduplicated by URL while collecting).

- **`http.GetText` always issues a GET, even if you set `RequestParams.Body`.** `RequestParams` grew a `Body` field (url-encoded form data, sent with `Content-Type: application/x-www-form-urlencoded`) for POST-based ajax endpoints like utoon.us's `admin-ajax.php`, but `Get`/`GetText` hardcode the `"GET"` method — passing `Body` to `GetText` silently sends a GET-with-a-body, which the server 400s. Use `http.Post`/`http.PostText` (mirrors `Get`/`GetText`) whenever a request needs `Body` set.
