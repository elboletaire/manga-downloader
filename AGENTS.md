...
- asmotoon.com (Asmodeus Scans) looks like it needs a browser (reader `<img class="myImage">` tags mostly show a shared `placeholder.svg` src, only swapped to the real CDN URL by `vanilla-lazyload` once scrolled into view) but plain HTTP already returns every image tag with a stable `uid` attribute holding the real per-page identifier — `getPlainHTMLImageURL()` prefers that over `src`/`data-src` when present, reconstructing `https://cdn.meowing.org/uploads/{uid}`. No Cloudflare, no login required even for chapters flagged with a coin cost in the chapter list (that gate only appeared to apply to the newest 1-2 chapters of each series). Before assuming a lazy-loaded reader needs a browser, check whether the un-swapped placeholder `<img>` still carries the real id in some other attribute.
