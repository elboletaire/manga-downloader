...
- Site-specific grabbers with their own logic: `inmanga.go`, `mangadex.go` (both API-based, language-aware), `mangabats.go` (JSON chapters API + js image variables), `mangafire.go` (open JSON API: `/api/titles/{hid}` + paginated `/chapters` + `/api/chapters/{id}` for pages, language-aware), `mangak.go` (mangak.io, the mangabuddy rebrand: Next.js site, full chapter list and page-image URLs parsed from the `__NEXT_DATA__` JSON blob — the visible HTML only lists ~6 chapters, and the JSON `number` field is a 1-based ordinal so the real number is parsed from the chapter name), `qimanga.go` (paginated JSON chapters API + SSR reader), `tcb.go` (Madara-based wordpress sites, i.e. lhtranslation.net), `leercapitulo.go` (plain HTML chapter list, but the reader page only decodes its obfuscated page-image blob client-side; uses `browser.GetHTMLWithLocalStorage` to flip the site's own "load all pages" preference before scraping the resulting `<img>` tags — no cloudflare, no cipher reversing needed), `stonescape.go` (stonescape.xyz: a Vue SPA whose series/reader routes render an empty `#app` div, but `PROBE_NETLOG=1` on the browse page reveals a wide-open, unauthenticated JSON API — `/api/series/by-slug/{slug}`, `/api/series/by-slug/{slug}/chapters` (unpaginated, full list) and `/api/chapters/{id}/pages` — no browser needed at all; images are plain webp direct-fetchable with no cookies/referer requirements. Note: its `robots.txt` explicitly `Disallow: /` for `ClaudeBot`/`anthropic-ai`/`Claude-Web` by name (on top of a generic `ai-train=no` signal) — the repo owner decided this user-driven download tool, run locally by end users like a browser, is not the crawler that directive targets, so support was added anyway).
