...
- Site-specific grabbers with their own logic: `inmanga.go`, `mangadex.go` (both API-based, language-aware), `mangabats.go` (JSON chapters API + js image variables), `mangafire.go` (open JSON API: `/api/titles/{hid}` + paginated `/chapters` + `/api/chapters/{id}` for pages, language-aware), `mangak.go` (mangak.io, the mangabuddy rebrand: Next.js site, full chapter list and page-image URLs parsed from the `__NEXT_DATA__` JSON blob — the visible HTML only lists ~6 chapters, and the JSON `number` field is a 1-based ordinal so the real number is parsed from the chapter name), `qimanga.go` (paginated JSON chapters API + SSR reader), `tcb.go` (Madara-based wordpress sites, i.e. lhtranslation.net), `leercapitulo.go` (plain HTML chapter list, but the reader page only decodes its obfuscated page-image blob client-side; uses `browser.GetHTMLWithLocalStorage` to flip the site's own "load all pages" preference before scraping the resulting `<img>` tags — no cloudflare, no cipher reversing needed), `vortexscans.go` (vortexscans.org, an Astro site: no Cloudflare, plain HTTP for everything. The series page only renders the ~20 most recent chapters as `<a>` tags, but the *full* chapter list is embedded as hydration state on an `<astro-island props="...">` attribute, devalue-style encoded — every value wrapped as `[0, value]` (plain/object, recurse) or `[1, [...]]` (array of wrapped elements), undefined as the 1-element `[0]`; `vsUnwrap()` undoes it. That "post" island's sibling top-level key `initialChap` (not nested inside `post`, despite the name and its physical proximity in the blob) holds all chapters, newest first. Reader pages need no such parsing: page images are plain `<img data-reader-page-image src="...">` tags in DOM order. Newest few chapters can be paywalled (coins/early access — `isLocked`/`isAccessible` fields in the same blob); their reader page simply renders zero page images, surfaced as a clear "might be locked" error rather than an empty CBZ).
