...
- **Sites keep moving/locking down.** zonatmo.com → taken down (Spanish police, Apr 2026) → successor zonatmo.org. colamanga → redirects to yoyomanga.com and now gates the reader behind their app (`APP观看该内容`) on top of client-side image encryption — not feasible, left unsupported (#30). mangabuddy.com → rebranded as mangak.io (supported, see `mangak.go`); the old mangabuddy.com domain now redirects to comizy.io, a different JS-rendered platform. sakuramangas.org (#90) → not feasible: its Cloudflare WAF blocks even our visible CDP-driven Chrome (two 5-minute headed attempts never cleared), and behind it sits a five-layer stack (rotating obfuscated `security.oby.js` keys, 29×SHA-256 proof-of-work headers, CSRF/challenge tokens, custom "AetherCipher" image-list encryption) that repeatedly broke and got the dedicated Tachiyomi/keiyoushi extension permanently delisted.
