...
- **hivetoons.org (VoidScans/HiveToons)** is a Next.js/Astro-hybrid site, but the full chapter list and all reader-page images are already present in the plain-HTTP server-rendered HTML (no CF, no JS needed) — Path A. The catch: the series page also embeds a "recently added" widget and a bare "continue reading" link reusing the identical `/series/{slug}/chapter-{n}` href shape as the real chapter-list rows, so a naive `a[href*="/chapter-"]` selector triples up on the ~20 most recent chapters. The real list's anchors carry a `.p-3` class the other widgets don't, so `Rows: a.p-3[href*="/chapter-"]` selects exactly the chapter rows with zero duplicates. Reader images use a clean `img[data-reader-page-image]` marker attribute.
