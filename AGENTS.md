...
- elftoon.com is the same mangastream/themesia theme as sushiscan.net (`ts_reader.run` images, already handled generically) but reachable over plain HTTP with no Cloudflare — Path A, not Path C. Its newest chapters are gem/coin-locked behind a modal (`href="#"` instead of a real URL), so `Rows` uses `:has(a.chapter-link-overlay[href^="http"])` to only list chapters that are actually free to read. Its `.chapternum` text also has literal multi-space padding baked into the HTML (browsers collapse it, raw scraping doesn't), which is why `FetchChapters` now runs chapter titles through `sanitizeTitle()` too (previously only the series title was sanitized).
