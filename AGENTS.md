...
- Site-specific grabbers with their own logic: `inmanga.go`, `mangadex.go` (both API-based, language-aware), `mangabats.go` (JSON chapters API + js image variables), `mangafire.go` (open JSON API: `/api/titles/{hid}` + paginated `/chapters` + `/api/chapters/{id}` for pages, language-aware), `mangak.go` (mangak.io, the mangabuddy rebrand: Next.js site, full chapter list and page-image URLs parsed from the `__NEXT_DATA__` JSON blob — the visible HTML only lists ~6 chapters, and the JSON `number` field is a 1-based ordinal so the real number is parsed from the chapter name), `qimanga.go` (paginated JSON chapters API + SSR reader), `tcb.go` (Madara-based wordpress sites, i.e. lhtranslation.net), `leercapitulo.go` (plain HTML chapter list, but the reader page only decodes its obfuscated page-image blob client-side; uses `browser.GetHTMLWithLocalStorage` to flip the site's own "load all pages" preference before scraping the resulting `<img>` tags — no cloudflare, no cipher reversing needed), `mangalib.go` (mangalib.me: Vue/Nuxt SPA behind a passive DDoS-Guard that never actually challenges plain GETs, backed by a wide-open JSON api on a separate host, `api2.mangalib.me` — `api.lib.social` from older writeups no longer resolves; page-image URLs from `/chapter?number=&volume=` are server-relative strings starting with a literal `//` that is *not* protocol-relative — no host follows it, it's just concatenated onto the CDN base url, `https://img2.imglib.info`, discovered via `/api/constants?fields[]=imageServers`; that CDN 403s if the request carries an `Origin` header at all, even matching the site's own — `Referer` alone is fine, which is exactly what `http.RequestParams` sends by default since `Origin` is opt-in; chapters with more than one scanlation team ("branch") still resolve correctly with a bare number+volume query, no `branch_id` needed, since the api defaults to the same team the site itself opens first).
