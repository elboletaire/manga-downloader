...
- `PlainHTML` (`grabber/plainhtml.go`) is a generic goquery scraper driven by a list of `SiteSelector` entries (CSS selectors for title/rows/chapter/image). It covers mangabuddy, tcbonepiecechapters.com (TCB Scans), asurascans.com and furyosociety.com. Selector order in the list matters because some sites have very similar markup.
- furyosociety.com runs FoOlSlide 2.3.3 (an old manga CMS): its chapter list markup duplicates every chapter as a `div.element.desktop` plus an `a.element.mobile` — only the mobile `<a>` is itself the reader link, so scoping `Rows` to `.fs-chapter-list a.element.mobile` avoids double-counting chapters. The reader page renders all page `<img>`s server-side in one go (no ajax pagination), so a plain `Image` selector is enough — no browser or JS-variable scraping needed.
