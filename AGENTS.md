...
- Site-specific grabbers with their own logic: `inmanga.go`, `mangadex.go` (both API-based, language-aware), `mangabats.go` (JSON chapters API + js image variables), `mangafire.go` (open JSON API: `/api/titles/{hid}` + paginated `/chapters` + `/api/chapters/{id}` for pages, language-aware), `mangak.go` (mangak.io, the mangabuddy rebrand: Next.js site, full chapter list and page-image URLs parsed from the `__NEXT_DATA__` JSON blob — the visible HTML only lists ~6 chapters, and the JSON `number` field is a 1-based ordinal so the real number is parsed from the chapter name), `qimanga.go` (paginated JSON chapters API + SSR reader), `tcb.go` (Madara-based wordpress sites, i.e. lhtranslation.net), `leercapitulo.go` (plain HTML chapter list, but the reader page only decodes its obfuscated page-image blob client-side; uses `browser.GetHTMLWithLocalStorage` to flip the site's own "load all pages" preference before scraping the resulting `<img>` tags — no cloudflare, no cipher reversing needed), `hijala.go` (en-hijala.com: no cloudflare, plain HTTP throughout, but the series page's Next.js RSC payload only server-renders chapter 1 plus the ~20 newest chapters, so the full list comes from a paginated JSON API at a separate `api.en-hijala.com` host keyed by a numeric `postId` that has to be scraped out of the series page's escaped RSC text with `postId\":(\d+)`, paginated with `&skip=N` in steps of 10 — `page=`/`limit=` params are silently ignored; on the reader page only the first ~6 `<img>` tags get a real `src` server-side, the rest are lazy-hydrated client-side, so pages are extracted with a regex over the raw HTML for `storage.en-hijala.com/upload/series/...` URLs (present for every page in the RSC payload) rather than a goquery `<img>` selector, sorted by the `page-NNNN` ordinal embedded in each filename).
